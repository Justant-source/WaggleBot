...
def _render_processing_detail_detached(posts: list, contents: dict) -> None:
    """PROCESSING 포스트를 파이프라인 진행 상태 테이블로 렌더링 (사전 로드된 Content 사용)."""
    if not posts:
        st.caption("해당 게시글 없음")
        return
...
def render() -> None:
    ...
    with _prog_hdr:
        st.header("⚙️ 진행 현황")
        st.caption("AI 워커 처리 상태 모니터링 (60초마다 자동 갱신)")
    with _prog_ref:
    ...
    # 전체 페이지를 fragment로 감싸 60초마다 자동 갱신
    @st.fragment(run_every="60s")
    def _progress_full():
        """진행현황 전체 자동 갱신 (60초 간격)."""
        with SessionLocal() as _ms:
            ...
            _total_processed = (
                (_counts.get(PostStatus.RENDERED, 0)
                 + _counts.get(PostStatus.UPLOADED, 0))
            )
            _total_failed = _counts.get(PostStatus.FAILED, 0)
        ...
        # 각 상태별 expander — 단일 세션으로 필요한 데이터 일괄 조회
        _status_with_data = [s for s in progress_statuses if _counts.get(s, 0) > 0]
        _all_status_posts: dict[PostStatus, list] = {}
        _all_fail_contents: dict[int, Content] = {}

        if _status_with_data:
            with SessionLocal() as _exp_session:
                for _st in _status_with_data:
                    _all_status_posts[_st] = (
                        _exp_session.query(Post)
                        .filter(Post.status == _st)
                        .order_by(Post.updated_at.desc())
                        .limit(10)
                        .all()
                    )
                # PROCESSING: Content도 함께 로드
                _proc_posts = _all_status_posts.get(PostStatus.PROCESSING, [])
                if _proc_posts:
                    _proc_ids = [p.id for p in _proc_posts]
                    _proc_contents = {
                        c.post_id: c
                        for c in _exp_session.query(Content)
                        .filter(Content.post_id.in_(_proc_ids))
                        .all()
                    }
                else:
                    _proc_contents = {}
                # FAILED: Content 일괄 사전 로드
                _fail_posts = _all_status_posts.get(PostStatus.FAILED, [])
                if _fail_posts:
                    _fids = [p.id for p in _fail_posts]
                    _all_fail_contents = {
                        c.post_id: c
                        for c in _exp_session.query(Content)
                        .filter(Content.post_id.in_(_fids))
                        .all()
                    }
        ...
        for status in progress_statuses:
            ...
            with st.expander(label, expanded=False):
                ...
                posts = _all_status_posts.get(status, [])
                if not posts:
                    st.caption("해당 게시글 없음")
                    continue
                ...
                if status == PostStatus.PROCESSING:
                    _render_processing_detail_detached(posts, _proc_contents)
                    continue
                ...
                for post in posts:
                    ...
                    with col1:
                        ...
                        if status == PostStatus.FAILED:
                            _fail_content = _all_fail_contents.get(post.id)
        ...
        # 실시간 통계 — 상단 카운트 재사용 (추가 DB 쿼리 없음)
        st.subheader("📊 실시간 통계")
        stat_col1, stat_col2, stat_col3 = st.columns(3)
        stat_col1.metric("대기 중", _total_collected)
        stat_col2.metric("완료", _total_processed)
        stat_col3.metric("실패", _total_failed)
