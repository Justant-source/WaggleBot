...
def build_chunking_system(*, extended: bool = False) -> str:
    ...
    return (
        ...
        "- 문장 길이와 리듬을 변주하세요 — 단, 리듬은 '문장 자체의 길이'로 만드는 것이지, '짧은 문장을 억지로"
        " 여러 줄로 쪼개는 것'이 아닙니다. 어떤 항목은 짧은 한 문장(1줄)으로 툭 끊고, 어떤 항목은 원래 긴"
        " 문장을 그대로 살려(1줄, 40자까지) 몰아치세요. 문장이 40자 안에서 끝나면 반드시 1줄입니다.\n"
        '- (X) 짧고 완결된 문장을 리듬감 때문에 억지로 2줄로 쪼갬 — "32년을 딴짓 없이" / "열심히만 살았어요"'
        ' (합치면 17자, 완결된 한 문장인데 왜 나눴는지 이유가 없음 — 기계적).\n'
        '- (O) 완결된 문장은 길이에 관계없이(40자까지) 한 줄에 담고, line_count=2는 그 문장이 정말'
        ' 40자를 넘고도 안 끝났을 때만 씁니다 — "32년을 딴짓 없이 열심히만 살았어요" (1줄, 17자, 완결).\n'
        ...
        "- 한 줄(lines 요소)은 40자 이내, 의미가 어색하게 끊기지 않게 하세요.\n"
        "- 문장이 40자 안에서 끝난다면 굳이 나누지 말고 한 줄로 담으세요 — 지금보다 짧게 여러 줄로"
        " 쪼개는 게 아니라, 한 호흡에 말할 수 있는 만큼은 한 줄에 담는 게 자연스럽습니다.\n"
        '  - (O) "정유사들은 다 똑같아" (한 줄, 문장이 끝남)   (X) "정유사들은" / "다 똑같아" (불필요한 분할)\n'
        "- 40자를 넘고 문장도 안 끝났을 때만 어절 단위로 나눠 같은 항목의 lines에 담고 line_count를 늘리세요.\n\n"
