...
#
# NOTE: title_suggestion 지시가 chunker.py("쇼츠 제목 50자 이내")와 다르게 "원문 제목 그대로"이지만,
#       실제 업로드 제목은 post.title 원문을 사용하므로(uploaders/uploader.py) 운영 영향 없음 — 의도적으로 미통일.
# ---------------------------------------------------------------------------
...
3. 끝까지 보게 만들기 (리텐션 — 가장 중요): 당신의 유일한 성공 지표는 시청자가 마지막 1초까지 보는 것입니다.
   (3-1) Hook[0~3초]: 아래 공식 중 하나로 첫 줄부터 꽂으세요. 첫 어절에 가장 강한 단어를 두고(끝에 두지 말 것), 구체 명사 또는 숫자를 1개 이상 반드시 넣으세요.
       ...
       (e) 1인칭 고백·후회형 — "그날 신고 안 한 거, 아직도 후회해요"
   ...
   (3-2) Hook-Payoff 계약: hook이 연 궁금증의 답(payoff)은 body 마지막 1/3 구간에서만 풀어주세요. 초반 블록에서 결말·정체 단서를 흘리면 즉시 이탈합니다. 단, hook이 약속한 답은 대본 안에서 반드시 줘야 합니다(어그로만 끌고 답 없이 끝내는 낚시 금지).
   (3-3) 에스컬레이션 체인: 블록을 "그리고/그래서"로 단순 나열하지 말고 "근데/알고 보니/문제는/여기서부터가"로 전환·고조하세요. 매 2~3블록마다 새 정보·반전·감정 전환 중 하나가 나오게.
       · (X) "갔어요. 그리고 봤어요. 그리고 말했어요"  · (O) "갔어요. 근데 문이 열려 있더라고요. 알고 보니…"
   (3-4) 블록 클리프행어: 긴장이 걸린 지점에서 블록을 일부러 미완으로 끊으세요 ("…열어봤는데", "도착한 곳은"). 단 어절 중간이 아닌 의미가 매달리는 지점에서.
   (3-5) 감정 낙차: payoff 직전 블록은 평온·일상 톤으로 낮췄다가 다음 블록에서 떨어뜨리세요 (평온→충격, 웃음→소름).
   (3-6) [중간] 클라이맥스 직전에 떡밥을 한 번 까세요.
     - 핵심: "지금부터예요" 같은 막연한 마무리는 쓰지 말고, 떡밥 안에 사연의 구체 단어(인물·사물·반전 대상)를 넣어 무엇이 터질지 살짝 흘리세요.
       (X) 막연(금지): "근데 진짜 어이없는 건 지금부터예요"
       (O) 구체: "근데 진짜 어이없는 건 시어머니가 한 말이에요" / "근데 그 카톡 한 줄이 다 뒤집었어요"
     - 아래는 mood별 '톤'일 뿐 — 문구를 복붙하지 말고 위처럼 사연 단어를 넣어 매번 새로 쓰세요:
       · shock/horror: 소름·등골 서늘   · humor: 빵 터지는·웃긴 포인트
       · touching/sadness: 코끝 찡한·할 말 잃은   · anger/controversy: 어이없는·기가 막힌   · daily/info: 반전·생각이 확 바뀐
   (3-7) Closer: 댓글 참여를 유도하는 한 문장으로 끝내세요. 우선순위 ① 편가르기·선택강요형("이거 제가 예민한 거예요?", "사이다예요 범죄예요?") ② 루프형(hook 질문 되받기) ③ 열린 질문(차선). 단 편가르기는 의견 대립 수준까지만, 혐오·정치 편가르기 금지. 아동·노약자·동물 등 약자에 대한 명백한 학대·폭력은 찬반 투표형 편가르기로 만들지 말고("훈육이냐 학대냐" 식 금지) 피해자 보호·공감 관점의 질문으로 대체하세요.
...
10. 고유명사·팩트 보존 (환각 금지): 한국어만 사용하고, 원문에 나오는 지명·고유명사를 함부로 바꾸거나 없는 사실을 지어내지 마세요.
    단, 실존 개인 식별정보 익명화: 사건·사고·의료·법적 분쟁 등 실제 피해자·가해자가 등장하는 글에서는 실명·나이·연락처·상세 주소 같은 식별정보를 가리세요(예: "강찬규 씨, 59세" → "60대 남성 A씨"). 서사·사실관계는 100% 보존하되 '누구인지'만 가립니다. 글쓴이 본인·가족 호칭(남편·시어머니 등)과 공인·기관·기업·지역명은 그대로 둡니다.
11. 민감 소재 처리: 성적·폭력·우울·자해 등 민감한 소재가 나와도 서사를 회피하거나 중간에 끝내지 마세요. 끝까지 다루되 표현은 원칙 1에 따라 완곡히 순화해 광고 친화적으로 바꾸세요(서사는 100% 유지, 단어만 순화).
12. body 항목 수: 최소 6개, 최대 30개(본문 1500자 이상일 때). 원문 분량에 비례하되 서사를 충분히 전달하도록 넉넉하게 작성하세요.
    ...
    *,
    ...
    try:
        with LLMCallTimer() as timer:
            # max_tokens=8192: body 30개 + 댓글 3개 한국어 JSON이 2048에 잘리지 않도록 상향
            # (transport.py의 api 보정은 2048 이상 값은 건드리지 않으므로 명시 필요)
