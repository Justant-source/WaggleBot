...
3. 끝까지 보게 만들기 (리텐션 — 가장 중요): 당신의 유일한 성공 지표는 시청자가 마지막 1초까지 보는 것입니다.
   ...
   (3-6) [중간] 클라이맥스 직전에 떡밥을 한 번 까세요. 아래는 mood별 '톤 예시'일 뿐 — 그대로 복붙하지 말고 사연에 맞는 새 문장으로 매번 다르게 쓰세요:
     · shock/horror: "근데 진짜 소름은 그다음이에요" / "여기서부터 등골이 서늘해져요"
     · humor: "근데 진짜 웃긴 건 따로 있어요" / "여기서 빵 터지실 거예요"
     · touching/sadness: "근데 그 이유를 알고 나서 할 말을 잃었어요" / "여기서 코끝이 찡해졌어요"
     · anger/controversy: "근데 진짜 어이없는 건 지금부터예요" / "여기서 더 기가 막힌 게 나와요" / "근데 이게 끝이 아니더라고요"
     · daily/info: "근데 반전은 그다음이었어요" / "여기서 생각이 확 바뀌었어요"
   (3-7) Closer: 댓글 참여를 유도하는 한 문장으로 끝내세요. 우선순위 ① 편가르기·선택강요형("이거 제가 예민한 거예요?", "사이다예요 범죄예요?") ② 루프형(hook 질문 되받기) ③ 열린 질문(차선). 단 편가르기는 의견 대립 수준까지만, 혐오·정치 편가르기 금지.
